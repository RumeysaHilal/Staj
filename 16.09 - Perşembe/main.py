...
def grafik():
    ...
    for i in range(0, 29):  # basilantus[] dizisinin uzunluğu adeddince döndürdüm
        basilantus1.append((basilantus[i][15:23]))  # Sadece harflerin eklenmesi için yazdım
    ...
    plt2.pie(basilmasayisi, labels=basilantus1, explode=explode, wedgeprops={'edgecolor': 'black'}, radius=1.2,
                  textprops={'fontsize': 10, 'color': "#065535"}, rotatelabels=45, autopct='%1.1f%%') # Dairesel grafiğin çizilmesi
    ...
    for i in range(0,29): #Basılma sayılarının yüzdeliklerini hesapladım, basılma sayısı ve basılan tuşla birlikkte listeye attım.
        per = round((basilmasayisi[i]*100)/len(dizi),1)
        tablo.append([basilantus1[i],basilmasayisi[i],per])
    bubble_sort(tablo) #Azalan şekilde sıralanmasını istediğimiz için azalan şekilde sıralayacak bir kabarcık sıralama algoritması kullandım.
    
    tab3=ttk.Frame(tabNot) # Diğer iki sekmede yaptığım gibi yeni bir sekme oluşturdum.
    tabNot.add(tab3,text="Pie Chart Table")
    tablo.insert(0,["Karakterler","Basılma Sayısı","Yüzdelik Değer"]) #Tablodaki başlıkları belirtmek için listenin ilk baışna başlıkları ekledim.
